...
pub(super) async fn handle_list(ctx: &CommandContext) -> Result<CommandResult> {
    ...
    let browser_count = entries
        .iter()
        .filter(|e| {
            e.browser_mcp.unwrap_or(false) || cc_browser::detection::is_browser_server(&e.name)
        })
        .count();
    ...
    for (label, bucket) in &by_scope {
        ...
        for entry in bucket {
            ...
            let tag = if entry.browser_mcp.unwrap_or(false)
                || cc_browser::detection::is_browser_server(&entry.name)
            {
                " [browser]"
            } else {
                ""
...
async fn build_status_from_discovery(cwd: &std::path::Path) -> Vec<McpServerStatusInfo> {
    let configs = match cc_mcp::discovery::discover_mcp_servers(cwd) {
        Ok(configs) => configs,
        Err(err) => {
    ...
    let manager_handle = cc_mcp::runtime::current_manager();
        ...
        .map(|cfg| {
            ...
            if let Some(client) = manager.as_ref().and_then(|m| m.clients.get(&cfg.name)) {
                let (state, error) = match &client.state {
                    ...
                    cc_mcp::McpConnectionState::Error(error) => {
