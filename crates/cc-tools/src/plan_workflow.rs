...
use cc_types::plan_workflow::{PlanWorkflowRecord, PlanWorkflowStatus};
...
pub fn load(cwd: &Path) -> Result<Option<PlanWorkflowRecord>> {
    let path = cc_config::paths::current_plan_workflow_file_path(cwd);
    if !path.is_file() {
        return Ok(None);
    }
...
pub fn enter_plan_mode_state(
    ...
    let plan_file = cc_config::paths::current_plan_file_path(cwd);
