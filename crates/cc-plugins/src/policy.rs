...
///
/// Consumes `cc_config::mdm::settings::ManagedPolicy` to make policy decisions.
...
impl PluginPolicyEnforcer {
    ...
    pub fn is_signature_required(policy: Option<&cc_config::mdm::ManagedPolicy>) -> bool {
        policy
    ...
    pub fn max_plugins(policy: Option<&cc_config::mdm::ManagedPolicy>) -> Option<u32> {
        policy
            .and_then(|p| p.plugin_install_policy.as_ref())
