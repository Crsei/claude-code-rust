...
/**
 * Ported from
 * `ui/examples/upstream-patterns/src/components/Spinner/useStalledAnimation.ts`.
 *
 * Transitions the spinner toward red when the assistant stops emitting
 * tokens for more than 3 seconds. Kept pure — no React timers, just
 * refs and the `time` passed in by the parent animation clock.
 *
 * Returns `{ isStalled, stalledIntensity }` where `stalledIntensity` is
 * smoothly eased from the previous value toward the target (0\u20131)
 * when `reducedMotion` is false, and snapped directly when it's true.
 */
