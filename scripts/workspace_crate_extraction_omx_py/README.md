# workspace_crate_extraction_omx_py 说明文档

`scripts/workspace_crate_extraction_omx_py/` 是 workspace crate extraction 批处理计划的 Python 实现包。它不是普通业务代码，也不是 Rust 功能模块；它是一个本地编排工具，用来把大规模 crate 拆分任务拆成可审查的 Codex 批次，执行 guard，写出可恢复的运行产物，并在通过时提交 batch。
...
- 将完整 workspace crate extraction 任务清单拆成小 batch，降低一次性改动风险。
...
scripts/run-workspace-crate-extraction-omx.ps1
    -> scripts/workspace_crate_extraction_omx.py
        -> workspace_crate_extraction_omx_py.cli.main()

scripts/workspace_crate_extraction_omx_supervisor.py
    -> workspace_crate_extraction_omx_py.supervisor.main()
```

`run-workspace-crate-extraction-omx.ps1` 是兼容 PowerShell 入口。它负责查找 `python` 或 `py -3`，再把参数转成 Python CLI 参数。真正的 runner 逻辑在 `cli.py` 中。

`workspace_crate_extraction_omx_supervisor.py` 是续跑控制入口。它不直接执行任务细节，而是读取已有 batch summary，生成 remaining tasks 文件，再调用 Python runner。
...
Runner 入口是 `workspace_crate_extraction_omx_py.cli.main()`，核心流程如下：
...
```powershell
python .\scripts\workspace_crate_extraction_omx.py [options]
```

| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `--tasks-file` | `docs/scripts/workspace-crate-extraction-omx-tasks-2026-05-10.txt` | 完整任务清单。 |
...
| `--output-root` | `target/codex-runs/workspace-crate-extraction-omx` | batch summary、failure、validation、final report 输出目录。 |
...
```text
target/codex-runs/workspace-crate-extraction-omx/final-validation/fmt.log
target/codex-runs/workspace-crate-extraction-omx/final-validation/check.log
target/codex-runs/workspace-crate-extraction-omx/final-validation/clippy.log
target/codex-runs/workspace-crate-extraction-omx/final-validation/test.log
```
...
Supervisor 入口是 `workspace_crate_extraction_omx_py.supervisor.main()`。它用于恢复中断或失败后的长任务队列。
...
```powershell
python .\scripts\workspace_crate_extraction_omx_supervisor.py [supervisor-options] -- [runner-options]
```

| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `--tasks-file` | `docs/scripts/workspace-crate-extraction-omx-tasks-2026-05-10.txt` | 完整任务清单。 |
| `--runner-script` | `scripts/workspace_crate_extraction_omx.py` | supervisor 调用的 runner。 |
| `--observed-output-root` | 可重复；默认 `target/codex-runs/workspace-crate-extraction-omx` | 历史 runner 输出目录。 |
| `--supervisor-output-root` | `target/codex-runs/workspace-crate-extraction-omx-supervisor` | supervisor 状态、日志和 remaining 文件输出目录。 |
...
```text
target/codex-runs/workspace-crate-extraction-omx-supervisor/
```
...
```powershell
python .\scripts\workspace_crate_extraction_omx.py --skip-commit --skip-final-validation
```
...
```powershell
python .\scripts\workspace_crate_extraction_omx_supervisor.py `
  --observed-output-root target/codex-runs/workspace-crate-extraction-omx `
  --observed-output-root target/codex-runs/workspace-crate-extraction-omx-supervisor `
  --plan-only
```
