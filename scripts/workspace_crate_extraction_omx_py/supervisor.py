...
DEFAULT_TASKS_FILE = "docs/scripts/workspace-crate-extraction-omx-tasks-2026-05-10.txt"
DEFAULT_RUNNER_SCRIPT = "scripts/workspace_crate_extraction_omx.py"
DEFAULT_OBSERVED_OUTPUT_ROOT = "target/codex-runs/workspace-crate-extraction-omx"
DEFAULT_SUPERVISOR_OUTPUT_ROOT = "target/codex-runs/workspace-crate-extraction-omx-supervisor"
...
def write_supervisor_report(
    ...
    lines = [
        "# Workspace crate extraction supervisor",
        "",
...
def run_attempt(command: Sequence[str], root: Path, log_file: Path) -> int:
    print()
    print("=== Supervised workspace-crate-extraction attempt ===")
...
def repair_runner(
    ...
    prompt = f"""The workspace-crate-extraction-omx runner exited before all tasks completed.
...
- Keep scripts/workspace_crate_extraction_omx.py and run-workspace-crate-extraction-omx.ps1 working.
- Do not run the long extraction plan as validation.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Supervise workspace-crate-extraction-omx and resume incomplete task queues.",
        epilog="Arguments after -- are passed to scripts/workspace_crate_extraction_omx.py.",
    )
