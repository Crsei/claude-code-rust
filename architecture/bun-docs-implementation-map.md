...
| Tools | `search-and-navigation.mdx` | 已实现 | Glob、Grep、ToolSearch、LSP、WebSearch、WebFetch 已接入；Glob 排序与 Bun 文档存在差异。 |
...
- Tools 的 V2 Tasks 已实现，但没有独立 `TodoWrite` 工具入口；`Glob` 排序与 Bun 文档描述不同。
