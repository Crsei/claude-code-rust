...
| Context | `project-memory.mdx` | 部分实现 | memory CRUD、`MEMORY.md` 入口索引、封闭 `user/feedback/project/reference` 类型元数据、`CLAUDE.md` 注入、Project / Global / Team memory 主提示词注入存在；Auto memory 已由 `auto_memory_enabled` 门控注入，最近 session-insights 会按 workspace、时间窗口、tag 和当前 session 排除规则回注；生命周期抽取已改用确定性 insight helper。 |
...
| Safety | `auto-mode.mdx` | 部分实现 | `PermissionMode::Auto`、fallback、classifier result adapter、危险 allow 规则运行时 strip/restore 接线和 Phase 2 shared classifier foundation 已存在；权限层可消费 fast / thinking 分类结果并移除宽泛 shell / Agent allow 规则，但 central permission flow 尚未在 Auto mode tool execution 时调用 classifier。 |
...
1. 优先确认 Safety 的未实现项：`allowedPrompts` 通用 LLM classifier、Auto mode classifier foundation 到 central permission flow 的调用接线；Windows OS-level sandbox 仅在故意裁剪复审条件触发时单独重开。
...
| 2026-05-07 | Safety / Shared classifier foundation | 已完成 Phase 2 classifier foundation；central permission flow 调用接线仍部分实现 | `permissions.autoMode` typed config、schema 与 merge tests 已补；`crates/claude-code-rs/src/safety/classifier.rs` 提供 prompt 构建、redaction、budget guard、strict JSON parsing、fast/thinking escalation、`QueryDeps` adapter 与 mock tests | `cargo test -p cc-config permissions_auto_mode -- --nocapture`，2 passed；`cargo test -p claude-code-rs safety::classifier::tests:: -- --nocapture`，9 passed |
