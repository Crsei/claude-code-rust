...
| Context | `compaction.mdx` | 部分实现 | 本地压缩、Session Memory Compact、boundary、手动 `/compact` / Session Memory Compact / 自动模型摘要 / 内部 snip/context-collapse preservedSegment 元数据、PTL 恢复与 hook 路径存在；feature gate、Partial Compact 与完整恢复语义仍未完全同构。 |
