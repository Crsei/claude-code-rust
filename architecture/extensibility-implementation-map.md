...
运行时已经能连接、初始化并管理 MCP 服务：`connect_all()` 与 `disconnect_all()` 在 `crates/cc-mcp/src/manager.rs:32-67` 和 `crates/cc-mcp/src/manager.rs:135-139`，`McpClient::connect()`、`initialize()`、`list_tools()`、`call_tool()`、`list_resources()`、`read_resource()` 分别位于 `crates/cc-mcp/src/client.rs:93-239`、`crates/cc-mcp/src/client.rs:273-401`。
