...
| 3.1 fallback wrapper | 已完成 | 2026-05-05 | `cargo test -p claude-code-rs test_fallback_model_retries_stream_start_capacity_error`，1 passed；`cargo test -p claude-code-rs query::loop_impl::loop_tests`，11 passed。 | `QueryParams::fallback_model` 现在会在 stream 建立前的 529 / overloaded / high-demand / capacity 失败上触发一次 fallback retry；`prompt-too-long` 仍优先走 reactive compact，stream 中途失败、tombstone 和 partial assistant 清理留给 3.2 / 3.6。 |
