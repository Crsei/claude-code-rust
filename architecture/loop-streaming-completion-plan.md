...
| 8.2 Google Gemini tool use / thinking | 已完成 | 2026-05-05 | `cargo test -p claude-code-rs api::google_provider::tests`，8 passed；`cargo test -p claude-code-rs test_capabilities_for_google_supports_tools_and_thinking`，1 passed。 | Gemini request builder 现在转换 Anthropic tools、tool_choice、thinkingConfig、assistant `tool_use` 与 user `tool_result`；stream adapter 将 `thought` parts 映射为 thinking block，将 `functionCall` 映射为 `ToolUse` + `input_json_delta`，并把 capability matrix 标记为 tool_use / thinking supported。 |
| 8.3 Vertex service-account JWT exchange | 已完成 | 2026-05-05 | `cargo test -p claude-code-rs api::vertex::tests`，12 passed；`cargo test -p claude-code-rs test_capabilities_for_vertex_are_supported`，1 passed；`cargo test -p claude-code-rs test_capabilities_for_foundry_are_unsupported`，1 passed。 | Vertex 认证链现在支持 `GOOGLE_APPLICATION_CREDENTIALS` service-account JSON：读取 key file，构造 RS256 JWT bearer assertion，交换 cloud-platform OAuth token；capability matrix 去掉 partial 状态并标记 Vertex supported。 |
