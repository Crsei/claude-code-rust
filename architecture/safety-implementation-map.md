...
| sandbox | [`crates/cc-config/src/settings.rs`](../crates/cc-config/src/settings.rs), [`crates/cc-sandbox/src/lib.rs`](../crates/cc-sandbox/src/lib.rs), [`crates/cc-sandbox/src/availability.rs`](../crates/cc-sandbox/src/availability.rs), [`crates/claude-code-rs/src/tools/exec/bash.rs`](../crates/claude-code-rs/src/tools/exec/bash.rs), [`crates/claude-code-rs/src/tools/exec/powershell.rs`](../crates/claude-code-rs/src/tools/exec/powershell.rs) | 部分实现 | Linux/macOS shell 沙箱、网络/路径预检、`failIfUnavailable`、`allowUnsandboxedCommands`、`excludedCommands`、`/sandbox` 命令都在；Windows OS-level 沙箱未实现。 |
...
- 权限引擎在 Auto mode 下默认放行，并保留拒绝计数回退到交互式确认的机制，见 [`crates/cc-permissions/src/rules.rs:138-155`](../crates/cc-permissions/src/rules.rs) 与 [`crates/cc-permissions/src/decision.rs:390-408`](../crates/cc-permissions/src/decision.rs)。
...
- Bun 文档里的 transcript classifier / two-stage 分类流水线没有在 cc-rust 中形成同等闭环；当前 Auto mode 只是权限引擎里的直接 allow + fallback 逻辑。
...
- `allowedCommands` 已进入 policy 结构，但当前 cc-rust binary 还没有把它接到 permission decision 闭环里。`crates/cc-sandbox/src/policy.rs:54-59` 已明确说明这是后续工作。
...
| --- | --- | --- | --- |
| `auto-mode.mdx` | `PermissionMode::Auto`、Auto mode fallback、可用性开关 | 部分实现 | [`crates/cc-types/src/permissions.rs:17-59`](../crates/cc-types/src/permissions.rs)，[`crates/cc-permissions/src/decision.rs:390-408`](../crates/cc-permissions/src/decision.rs)，[`crates/claude-code-rs/src/startup/runtime_config.rs:119-122`](../crates/claude-code-rs/src/startup/runtime_config.rs) |
...
| `sandbox.mdx` | shell 沙箱、网络/路径预检、`/sandbox`、`failIfUnavailable`、`excludedCommands` | 部分实现 | [`crates/cc-sandbox/src/lib.rs:11-28`](../crates/cc-sandbox/src/lib.rs)，[`crates/cc-sandbox/src/availability.rs:1-128`](../crates/cc-sandbox/src/availability.rs)，[`crates/claude-code-rs/src/tools/exec/bash.rs:279-407`](../crates/claude-code-rs/src/tools/exec/bash.rs) |
...
| `allowedCommands` 自动放行到 permission decision | 部分实现 | policy 层已经有 `allowedCommands`，但注释明确说 cc-rust binary 还没把它接到决策闭环里，见 [`crates/cc-sandbox/src/policy.rs:54-59`](../crates/cc-sandbox/src/policy.rs) 。 |

## 后续动作

1. 如果目标是继续向 Bun 对齐，先补 `allowedCommands` 到权限决策闭环，再决定是否进入 `workspace` 自动放行。
