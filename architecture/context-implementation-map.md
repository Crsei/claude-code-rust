...
| `token-budget.mdx` | [`cc-utils/src/tokens.rs`](F:/AIclassmanager/cc/rust/crates/cc-utils/src/tokens.rs), [`cc-compact/src/auto_compact.rs`](F:/AIclassmanager/cc/rust/crates/cc-compact/src/auto_compact.rs), [`claude-code-rs/src/query/token_budget.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/query/token_budget.rs#L9), [`claude-code-rs/src/api/client/mod.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/api/client/mod.rs) | 部分实现 | 预算判断、续跑逻辑、`CLAUDE_CODE_MAX_CONTEXT_TOKENS` 与 `[1m]` 窗口解析存在；`estimate_context_usage()` 仍提供离线启发式诊断，API 层已新增 Anthropic/Azure/Gemini provider exact 计数诊断路径，auto-compact 也会在启发式估算接近阈值时对支持 provider 执行 exact fallback；剩余差异是 OpenAI/Bedrock/Vertex provider parity。 |
...
- `部分实现`。
- 当前实现已经有预算判断、动态窗口解析、结构化启发式诊断、恢复动作、Anthropic/Azure/Gemini provider exact 诊断入口，以及 near-threshold auto-compact exact fallback；仍是 `部分实现`，因为 OpenAI/Bedrock/Vertex provider parity 还没有闭环。
...
- token 预算已经接入主循环：任务预算继续/停止、max_output_tokens 恢复、自动压缩阈值和 `TokenUsageReport` 结构化启发式诊断都不是占位；Anthropic/Azure/Gemini provider exact 诊断入口已存在，auto-compact 阈值也会在 near-threshold 情况下使用 exact fallback。[`token_budget.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/query/token_budget.rs#L9), [`loop_helpers.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/query/loop_helpers.rs#L202), [`tokens.rs`](F:/AIclassmanager/cc/rust/crates/cc-utils/src/tokens.rs#L141), [`client/mod.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/api/client/mod.rs)
...
- `token-budget.mdx`：有预算判断、动态窗口解析、结构化启发式诊断、恢复、Anthropic/Azure/Gemini provider exact 诊断入口和 auto-compact near-threshold exact fallback；OpenAI/Bedrock/Vertex provider parity 仍未完成。[`tokens.rs`](F:/AIclassmanager/cc/rust/crates/cc-utils/src/tokens.rs), [`client/mod.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/api/client/mod.rs), [`google_provider.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/api/google_provider.rs)
...
- 在本次 `context` 范围内，仍未完成 OpenAI/Bedrock/Vertex provider exact parity。Anthropic/Azure/Gemini 计数入口与 near-threshold auto-compact exact fallback 已进入 `部分实现` 范围。[`tokens.rs`](F:/AIclassmanager/cc/rust/crates/cc-utils/src/tokens.rs), [`client/mod.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/api/client/mod.rs)
...
2. 如果要继续对齐 `token-budget` 语义，下一步应重新评估 OpenAI/Bedrock/Vertex provider exact parity，并为可支持 provider 补请求形状测试。
