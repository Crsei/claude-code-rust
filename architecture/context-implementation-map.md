...
- `部分实现`。
- `CLAUDE.md`、Project / Global / Team memory 和受 `auto_memory_enabled` 门控的 Auto memory 都已经接入主提示词；剩余差异是 `SessionMemoryService` 的 insight 落盘没有自动回注到系统提示词。
...
- 记忆存储、命令面和主提示词注入已经实现：`cc-session::memdir` 支持四个 scope 的 CRUD 和搜索，`/memory` 也能查看、编辑和打开这些目录；Project / Global / Team memory 会进入 `# Memory Context`，Auto memory 会在 `auto_memory_enabled` 开启时进入同一段落。[`memdir.rs`](F:/AIclassmanager/cc/rust/crates/cc-session/src/memdir.rs#L111), [`commands/memory.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/commands/memory.rs#L41), [`system_prompt.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/engine/system_prompt.rs)
...
- `project-memory.mdx`：记忆 CRUD、`CLAUDE.md` 注入、Project / Global / Team memory 主提示词注入和 `auto_memory_enabled` 门控的 Auto memory 注入都存在；session-insights 自动回注仍未完整闭环。[`memdir.rs`](F:/AIclassmanager/cc/rust/crates/cc-session/src/memdir.rs#L248), [`system_prompt.rs`](F:/AIclassmanager/cc/rust/crates/claude-code-rs/src/engine/system_prompt.rs)
...
1. 如果要继续对齐 Bun 的 `project-memory` 语义，下一步应决定 `SessionMemoryService` 的 insight 是否要自动回注到主提示词或记忆检索链路。
