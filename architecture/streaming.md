...
```text
Provider HTTP/SSE 或 AWS EventStream
...
- Bedrock AWS EventStream 解码：`crates/claude-code-rs/src/api/bedrock.rs`
...
- Provider 适配层已把 Anthropic、OpenAI-compatible、Google、Vertex、Bedrock AWS EventStream 接入同一流式接口。
...
| P2 | Google tool use / thinking、Vertex service-account JWT exchange 等 provider 能力仍未补齐；Bedrock 仍缺真实 AWS provider/e2e 覆盖。 | 多 provider 行为还不是 full-build 对齐状态。 |
...
3. 再扩展 provider：Google tool use / thinking、Vertex service-account JWT exchange、Azure 命名与能力矩阵，并为 Bedrock EventStream 补真实 provider/e2e 覆盖。
