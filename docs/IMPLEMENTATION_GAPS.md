...
| PowerShell parse-error fail-closed 子集 | 已补齐子项 | `crates/cc-permissions/src/dangerous.rs` 在执行安全门中拒绝明显未闭合 single/double quote、paren、brace 与 type literal delimiter，以及明显 mismatched closing delimiter；该子项覆盖常见 parse-error fail-closed，完整 PowerShell 原生 AST parser fidelity 仍单列在 §2.2 |
