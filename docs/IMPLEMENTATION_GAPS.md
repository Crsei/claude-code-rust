...
1. **BashTool 剩余安全/沙箱复核**：对照 `src/tools/BashTool/**` 与 PowerShell validator，补齐 dynamic command name、script block/subexpression、splatting、member/static method/type literal、expandable string 等剩余 AST/security 检查；同时把 Windows OS-level sandbox 的可实现边界写清楚，能实现的落代码，不能实现的移入 §7 Intentional 裁剪。
