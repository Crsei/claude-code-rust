...
| WebFetch | JS 渲染、Cookie 管理、代理支持；redirect budget / cross-host redirect diagnostic 与 Content-Type 基础分发子项已落地 |
...
4. **WebFetch browser-grade 能力**：按 `architecture/mvp-optimization-plans/MVP-009-web-fetch-browser-grade-plan.md` 逐步补 JS 渲染、Cookie jar 与代理支持；redirect budget / cross-host redirect diagnostic 和 Content-Type 基础分发已完成。
