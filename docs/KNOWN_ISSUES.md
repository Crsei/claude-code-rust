...
| UI-011 | 中 | Fixed | Rust TUI mouse focus + subagent task alias | 用户反馈鼠标选中输入框时滚轮应翻 prompt 历史、选中聊天区域时滚轮应翻历史消息；聊天框中要求调用 subagent 时模型可能发出上游 `Task` 工具名并显示调用失败。 | App 记录最新 prompt/message 区域并按鼠标命中区域切换滚轮目标；prompt 区滚轮走输入历史，message 区滚轮走 session scroll。Agent runtime 同时注册 `Agent` 和上游兼容 `Task` 工具名，`Task` 复用同一 subagent 实现。 |
