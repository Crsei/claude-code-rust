...
| PowerShell parse-error fail-closed subset | `crates/cc-permissions/src/dangerous.rs`, `tools/execution/tests.rs` | `cargo test -p cc-permissions powershell_obvious_parse_errors`; `cargo test -p cc-permissions dangerous`; `cargo test -p claude-code-rs test_powershell_dangerous_command_blocked` | 执行安全门拒绝明显未闭合 quote、paren、brace、type literal delimiter 与 mismatched closing delimiter，避免 PowerShell 语法错误绕过静态安全检查 |
