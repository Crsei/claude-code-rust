...
| P7.5 | /config | `commands/config_cmd.rs` | ✅ | ~230 | 6 | show/set/reset 完整实现 |
...
| P7.16 | /fast | `commands/fast.rs` | ✅ | ~200 | 7 | 快速模式切换 + 自动模型切换 + 状态查询 |
...
| # | 模块 | 文件 | 状态 | 行数 | 测试 | 说明 |
|---|------|------|------|------|------|------|
| P8.1 | Agent | `tools/agent.rs` | ✅ | 323 | 5 | 子 QueryEngine 派生 + 递归深度限制 |
| P8.2 | PlanMode | `tools/plan_mode.rs` | ✅ | 432 | 8 | 真实状态转换: save/restore pre_plan_mode, 验证 agent 上下文, 读权限检查 |
| P8.3 | Worktree | `tools/worktree.rs` | ✅ | 724 | 8 | 真实 git worktree: 创建/清理, 变更检测 (status+rev-list), 失败关闭安全, slug 验证 |
| P8.4 | Skill | `tools/skill.rs` | ✅ | 454 | 7 | 真实技能查找/扩展/注入: 注册表查询, 参数替换, new_messages 注入, inline/fork 上下文 |
...
| # | 模块 | 文件 | 状态 | 说明 |
|---|------|------|------|------|
| P12.1 | WebFetch | `tools/web_fetch.rs` | ✅ | ~320 行; HTML→text, 缓存, URL 验证, 截断 |
| P12.2 | WebSearch | `tools/web_search.rs` | ✅ | ~310 行; Brave Search API, 域名过滤 |
| P12.3 | LSP 工具 | `tools/lsp.rs` | ✅ | ~460 行; 9 种操作, 回退文本分析 |
| P12.4 | LSP 服务 | `lsp_service/mod.rs` | 🔧 | ~190 行; 服务器配置/状态, API 占位 |
| P12.5 | 技能系统 | `skills/` (3 文件) | ✅ | ~520 行; 加载/注册/5个内置技能 |
| P12.6 | 插件系统 | `plugins/` (3 文件) | ✅ | ~520 行; manifest/loader/注册 |
...
| P14A.5 | utils/messages | `utils/messages/` | 扩展 `utils/messages.rs` | 450 | ✅ 消息格式化, 截断, 摘要, 统计 |
| P14A.6 | keybindings | `keybindings/` (14 文件) | `ui/keybindings.rs` | 290 | ✅ 快捷键注册, 上下文解析, 自定义绑定 |
| P14A.7 | vim 模式 | `vim/` (5 文件) | `ui/vim.rs` | 560 | ✅ Normal/Insert/Visual, hjkl, dd/yy/p/w/b |
| P14A.8 | 迁移系统 | `migrations/` (11 文件) | `session/migrations.rs` | 220 | ✅ 版本检测, v1→v2→v3 迁移链 |
| P14A.9 | 任务子系统 | `tasks/` (12 文件) | `tools/tasks.rs` | 430 | ✅ TaskStore + 6 工具 (Create/Get/Update/List/Stop/Output) |
| P14A.10 | 内存系统 | `memdir/` (8 文件) | `session/memdir.rs` | 280 | ✅ CRUD + 搜索 + 上下文注入 |
...
| P14B.18 | /fast | `commands/fast.rs` | ✅ | 快速模式切换 + 状态 |
...
> **注**: 命名对齐见 [`UNIMPLEMENTED_CHECKLIST.md`](UNIMPLEMENTED_CHECKLIST.md)。

### Phase 14C: 缺失工具补全

> 整合 UNIMPLEMENTED_CHECKLIST 中所有未实现工具。

**已规划工具 (待完成)**

| # | 工具 | 预估行数 | 优先级 | 说明 |
|---|------|---------|--------|------|
| P14C.1 | SendMessage | ~80 | 高 | Agent 间通信 |
| P14C.2 | EnterPlanMode / ExitPlanMode | ~60 | 高 | 计划模式工具对 |
| P14C.3 | EnterWorktree / ExitWorktree | ~80 | 中 | git worktree 工具对 |
| P14C.4 | Skill | ~60 | 中 | 技能调用 |
| P14C.5 | LSP | ~200 | 中 | 需 lsp feature |
| P14C.6 | PowerShell | ~100 | 中 | Windows 支持 |
| P14C.7 | Sleep | ~20 | 低 | tokio::time::sleep |
| P14C.8 | SendUserMessage (Brief) | ~30 | 低 | 输出简化 |
| P14C.9 | Config | ~50 | 低 | 设置修改 |
| P14C.10 | REPL | ~100 | 低 | 嵌入式 REPL |
| P14C.11 | WebFetch | ~100 | 低 | 需 network feature |
| P14C.12 | WebSearch | ~100 | 低 | 需 network feature |
| P14C.13 | RemoteTrigger | ~80 | 低 | 需 network |
| P14C.14 | CronCreate/Delete/List | ~120 | 低 | 需 network |
| — | MCP | ✅ 已完成 | — | McpToolWrapper |

**新增发现工具 (UNIMPLEMENTED_CHECKLIST)**
...
```
TeamCreate, TeamDelete, CtxInspect, OverflowTest,
...
实现完成度:
  ...
  Phase 7  命令系统       ██████████ 100% (18/18 完整)
  Phase 8  高级工具       ██████████ 100% (4/4 完整)
...
| 任务 | 文件 | 预估 | 状态 | 依赖 |
|------|------|------|------|------|
| /config 完整实现 | `commands/config_cmd.rs` | ~230 行 | ✅ 已完成 | 无 |
| /fast 快速模式 | `commands/fast.rs` | ~200 行 | ✅ 已完成 | 无 |
| utils/messages 扩展 | `utils/messages.rs` | ~450 行 | ✅ 已完成 | 无 |
| keybindings 系统 | `ui/keybindings.rs` | ~290 行 | ✅ 已完成 | 无 |
| vim 模式 | `ui/vim.rs` | ~560 行 | ✅ 已完成 | 无 |
| session migrations | `session/migrations.rs` | ~220 行 | ✅ 已完成 | 无 |
| memdir 内存系统 | `session/memdir.rs` | ~280 行 | ✅ 已完成 | 无 |
| 插件/技能系统 | `plugins/`, `skills/` | ~1040 行 | ✅ 已完成 | 无 |
| LSP 集成 | `tools/lsp.rs` + `lsp_service/` | ~650 行 | ✅ 已完成 | lsp feature |
| PlanMode/Worktree 工具 | `tools/plan_mode.rs`, `worktree.rs` | ~1160 行 | ✅ 已完成 | 无 |
| Skill 工具 | `tools/skill.rs` | ~455 行 | ✅ 已完成 | skills/ |
| AWS Bedrock (填充接口) | `api/providers.rs` | 150 行 | 📦 接口保留 | aws-sdk |
| GCP Vertex (填充接口) | `api/providers.rs` | 150 行 | 📦 接口保留 | gcp_auth |
| OAuth (填充接口) | `auth/mod.rs` | 300 行 | 📦 接口保留 | oauth2 |
| WebFetch / WebSearch | `tools/web_*.rs` | ~630 行 | ✅ 已完成 | reqwest |
...
├── remote/                      ← remote/
├── session/                     ← memdir/ + 会话相关
│   ├── migrations.rs            ← ✅ 版本迁移系统
│   └── memdir.rs                ← ✅ CLAUDE.md 记忆系统
├── tasks/                       ← 待新增: tasks/
├── tools/                       ← tools/ (14 个工具 + 基础设施)
├── types/                       ← types/ + state/ + schemas/
├── ui/                          ← components/ + ink/ + screens/
│   ├── keybindings.rs           ← ✅ 快捷键注册系统
│   └── vim.rs                   ← ✅ Vim 模态编辑
├── utils/                       ← utils/ (bash, git, shell, messages 等)
├── skills/                      ← ✅ skills/ (loader, bundled, 注册)
├── plugins/                     ← ✅ plugins/ (manifest, loader, 注册)
...
2. **可测试**: 437 个测试覆盖所有核心路径，QueryDeps trait 允许完整 mock
