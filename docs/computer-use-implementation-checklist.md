...
- [ ] 为 headless/TUI 增加最小可见性
  - [src/ipc/protocol.rs](../src/ipc/protocol.rs)：`ToolResult` 消息建议增加可选结构化字段或图片预览元信息
  - [src/ui/messages.rs](../src/ui/messages.rs)：至少能把图片型 tool result 渲染成 `[image]`
...
- [ ] 扩展 [src/permissions/decision.rs](../src/permissions/decision.rs) 的 matcher
  ...
  - 支持未来原生工具名
- [ ] 为 Computer Use 定义单独的危险等级
  - 截图：中风险
  - 点击/输入/快捷键：高风险
  - 打开应用、切换窗口、剪贴板读写：高风险
- [ ] 在 [src/types/tool.rs](../src/types/tool.rs) / permission context 中增加“会话级授权”概念
  - 避免每次点击都弹权限框
...
- [ ] session-level request_access
- [ ] 权限 matcher / dangerous 分类
- [ ] TUI / headless 显示优化
