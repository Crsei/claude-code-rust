...
- 权限系统 (4 种模式 + 危险检测)
- **完整终端 TUI** (ratatui + crossterm):
  - 全屏交替屏幕渲染
  - Markdown 消息渲染 (标题、粗体、斜体、代码块、列表、链接)
  - 工具调用和结果展示
  - Thinking 块折叠显示
  - 权限对话框覆盖层
  - Braille 点阵旋转动画
  - 输入历史 (Up/Down 浏览)
  - 可滚动消息区域 (PageUp/PageDown)
  - 状态栏 (模型、消息数、费用、状态)
  - Vim 编辑模式 (Normal/Insert/Visual, d/y/c 操作符, w/b/e 移动)
  - 可自定义键绑定系统
  - Diff 渲染 (增/删/上下文行着色)
  - 23 种预定义主题样式
- 会话持久化与恢复
