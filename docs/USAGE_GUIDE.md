...
```bash
# 仅启用 Anthropic API
cargo build --release --features api-anthropic

# 启用 API + 认证
cargo build --release --features "api-anthropic,auth"
...
通过 Cargo features 按需启用功能模块，控制编译依赖:

| Feature | 说明 | 依赖 |
|---------|------|------|
| `network` | 基础网络功能 | reqwest, eventsource-stream, tokio-tungstenite |
| `api-anthropic` | Anthropic API 直连 | network |
| `api-bedrock` | AWS Bedrock 提供商 | network, aws-sdk |
| `api-vertex` | GCP Vertex AI 提供商 | network, gcp_auth |
| `auth` | 认证系统 (Keychain/OAuth) | keyring, jsonwebtoken, oauth2 |
| `mcp` | Model Context Protocol | network |
| `lsp` | LSP 代码导航集成 | lsp-types |
| `tree-sitter-ast` | AST 解析 | tree-sitter |
| `full` | 启用所有功能 | 以上全部 |

### 默认构建

不启用任何 feature，纯本地/离线模式。核心工具 (文件操作、搜索、Bash) 全部可用。
...
# 企业部署 (Bedrock + 认证)
cargo build --release --features "api-bedrock,auth"
...
| 功能 | 状态 | 说明 |
|------|------|------|
| API 调用 | 脚手架 | `api/client.rs` HTTP 请求未接入 |
| OAuth 认证 | 接口保留 | 接口已定义 (`dead_code`)，流程未实现 |
| WebFetch / WebSearch | 脚手架 | feature-gated 占位 |
| MCP 客户端 | 脚手架 | 5 个方法均为 stub |
| 远程会话 | 脚手架 | 未实现 |
