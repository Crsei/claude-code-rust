...
- [ ] `cargo test --features network` 编译通过
...
- [ ] `QueryEngineDeps.call_model()` 不再 bail
- [ ] 集成测试 (手动): `ANTHROPIC_API_KEY=sk-... cargo run --features network -- -p "say hi"` 输出文本
...
# 2. 基本对话
ANTHROPIC_API_KEY=sk-... cargo run --features network -- -p "What is 2+2?"
# 预期: 输出包含 "4"

# 3. 工具使用
ANTHROPIC_API_KEY=sk-... cargo run --features network -- -p "List files in /tmp"
# 预期: 工具调用 Bash → 返回文件列表

# 4. 多轮对话 (REPL)
ANTHROPIC_API_KEY=sk-... cargo run --features network
> What is Rust?
# 预期: 输出 Rust 介绍
> /compact
# 预期: 压缩统计
