...
| `authProfiles`                    | cc-rust grouped login/provider settings. Use `codex` for OpenAI Codex, `anthropic` for official Claude Code/Anthropic auth, and `custom` for Anthropic-compatible custom APIs. Profile `env` values may contain provider-specific secrets when local policy allows plaintext settings.                                                                                                                                                                                                                                                                      | `{"codex":{"backend":"codex","apiProvider":"openai-codex","model":"gpt-5.4"}}`                                                |
