...
| `activeAuthProfile`               | cc-rust profile name selected from `authProfiles`. The active profile projects its `backend`, `apiProvider`, `model`, `availableModels`, `baseUrl`, `apiKey`, and `env` into the runtime settings before environment overrides are applied.                                                                                                                                                                                                                                                                                                                | `"codex"`                                                                                                                      |
| `authProfiles`                    | cc-rust grouped login/provider settings. Use `claude_code` for Claude Code / Anthropic-compatible auth, `codex` for OpenAI Codex, `openai` for OpenAI API, and `custom` only for future third-party profiles that do not map to a built-in provider. Legacy `anthropic` profiles are still read for compatibility but new writes use `claude_code`. Profile `env` values may contain provider-specific secrets when local policy allows plaintext settings.                                                                                                          | `{"claude_code":{"backend":"native","apiProvider":"anthropic","model":"SOTA"},"codex":{"backend":"codex","apiProvider":"openai-codex","model":"gpt-5.4"}}` |
...
| `availableModels`                 | Restrict which models users can select via `/model`, `--model`, Config tool, or `ANTHROPIC_MODEL`. cc-rust accepts `SOTA`, `MOTA`, `FOTA`, or full model IDs here; removed legacy aliases `opus`, `sonnet`, and `haiku` are rejected instead of being sent to providers. See [Restrict model selection](/en/model-config#restrict-model-selection)                                                                                                                                                                                                          | `["MOTA", "FOTA"]`                                                                                                             |
| `sotaModel`                       | cc-rust override for the neutral `SOTA` alias. When set, `/model SOTA`, `model: "SOTA"`, and `availableModels: ["SOTA"]` resolve to this model before provider defaults or built-in fallback constants.                                                                                                                                                                                                                                                                                                                                                   | `"gpt-5.5"`                                                                                                                    |
| `motaModel`                       | cc-rust override for the neutral `MOTA` alias. This also affects the default `MOTA` fallback used by startup and fast-mode defaults when no more specific model is configured.                                                                                                                                                                                                                                                                                                                                                                           | `"gpt-5.4-mini"`                                                                                                               |
| `fotaModel`                       | cc-rust override for the neutral `FOTA` alias. Use this to keep the fast/lightweight picker entry provider-specific without editing code.                                                                                                                                                                                                                                                                                                                                                                                                                  | `"gpt-5.3-codex-spark"`                                                                                                        |
...
| `model_reasoning_effort`          | Codex/OpenAI Responses reasoning effort. Accepts `"none"`, `"minimal"`, `"low"`, `"medium"`, `"high"`, or `"xhigh"` and maps directly to `reasoning.effort` for `openai-codex` requests. Use this instead of token budgets for Codex models.                                                                                                                                                                                                                                                                                                               | `"high"`                                                                                                                       |
